...
TRACKS_FOLDER = os.path.join(BASE_PATH, "Tracks")
...
def save_score(song_name, score, master):
    ...
    if len(leaderboard) < 15 or score > leaderboard[-1][1]:
...
class KaraokeApp(tk.Tk):
    def __init__(self):
        ...
        self.iconbitmap("Karaoke_icon.ico")
...
class MenuFrame(tk.Frame):
    def __init__(self, master):
        ...
        add_song_btn = tk.Button(self, text="+", font=("Arial", 20, "bold"), bg="white", command=self.open_add_song_popup)
        add_song_btn.pack(side="bottom", pady=20)

    def open_add_song_popup(self):
        ...
        tk.Label(popup, text="Folder name for the song:", font=("Arial", 12)).pack(pady=(15, 5))
        folder_entry = tk.Entry(popup, font=("Arial", 12))
        folder_entry.pack(pady=5)

        paths = {"audio": None, "lyric": None, "pitch": None, "image": None}
        labels = {}

        def select_file(types, key):
            path = filedialog.askopenfilename(filetypes=types)
            if path:
                paths[key] = path
                labels[key].config(text=f"Selected: {os.path.basename(path)}")

        tk.Button(popup, text="Select audio file", command=lambda: select_file([("Audio", "*.mp3 *.m4a *.wav")], "audio")).pack(pady=5)
        labels["audio"] = tk.Label(popup, text="", font=("Arial", 10))
        labels["audio"].pack()

        tk.Button(popup, text="Select lyrics file (.lrc)", command=lambda: select_file([("LRC", "*.lrc")], "lyric")).pack(pady=5)
        labels["lyric"] = tk.Label(popup, text="", font=("Arial", 10))
        labels["lyric"].pack()

        tk.Button(popup, text="Select pitch file (.txt)", command=lambda: select_file([("Pitch", "*.txt")], "pitch")).pack(pady=5)
        labels["pitch"] = tk.Label(popup, text="", font=("Arial", 10))
        labels["pitch"].pack()

        tk.Button(popup, text="Select cover image (optional)", command=lambda: select_file([("Images", "*.png *.jpg *.jpeg")], "image")).pack(pady=5)
        labels["image"] = tk.Label(popup, text="", font=("Arial", 10))
        labels["image"].pack()
        ...
        def add_song():
            ...
            if not folder_name:
                status_label.config(text="Please enter a folder name.")
                return

            required_files = ["audio", "lyric", "pitch"]
            for key in required_files:
                if not paths[key]:
                    status_label.config(text=f"Please select a {key} file.")
                    return

            new_folder_path = os.path.join(TRACKS_FOLDER, folder_name)
            
            if os.path.exists(new_folder_path):
                status_label.config(text="A song with this name already exists.")
                return
            ...
            for key, src_path in paths.items():
                if src_path:
                    ...
                    try:
                        ...
                        status_label.config(text=f"Failed to copy {key} file.")
                        return

            status_label.config(text="Song added successfully!", fg="green")
            self.load_song_list()

        tk.Button(popup, text="Submit", font=("Arial", 12), command=add_song).pack(pady=15)
        ...
        for folder_name in os.listdir(TRACKS_FOLDER):
            ...
            if os.path.isdir(folder_path):
                ...
                if any(f.endswith(('.mp3','.m4a')) for f in files) and any(f.endswith('.lrc') for f in files):
                    ...
                    image_file=  next((f for f in files if f.lower().endswith((".jpeg",".png",".jpg"))),None) 
                    if image_file:  
...
class KaraokeFrame(tk.Frame):
    def __init__(self, master):
        ...
        self.score_label = tk.Label(self, text="Score: 0", font=("Arial", 18), bg="white")
    ...
    def stop_song(self,save_score_flag = True):
        self.playing = False
        self.mic_running = False  
    ...
    def back_to_menu(self):
        self.stop_song(save_score_flag=False)
    ...
    def listen_to_mic(self):   
        ...
        BUFFER_SIZE = 2048  # Frames per buffer (audio chunk size) 
